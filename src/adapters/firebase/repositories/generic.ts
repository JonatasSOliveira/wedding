...
export abstract class GenericFirebaseRepository<CreateDTO> {
  ...
  public async create(data: CreateDTO, user: AuthenticatedUserResponseDTO): Promise<void> {
    await addDoc(
      this.col,
      { ...data, user_id: user.id, created_at: Timestamp.now(), updated_at: Timestamp.now(), deleted_at: null }
    )
  }
}